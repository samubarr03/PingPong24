...
/*** TO BE DONE START ***/
	if(clock_gettime(CLOCK_TYPE, &send_time)){
		fail_errno("Error receiving current time with send_time");
	}
...
/*** TO BE DONE START ***/
	if(clock_gettime(CLOCK_TYPE, &recv_time)){
		fail_errno("Error receiving current time with recv_time");
	}
...
/*** TO BE DONE START ***/
	...
	ping_socket = socket(gai_hints.ai_family, gai_hints.ai_socktype, gai_hints.ai_protocol);
	fcntl(ping_socket, F_SETFL, O_NONBLOCK); // Imposta la modalità non bloccante
...
if(connect(ping_socket, pong_addrinfo->ai_addr, pong_addrinfo->ai_addrlen) == -1)
    fail_errno("UDP Ping could not connect");
